...
# Constantes de Conversão (Idealmente, estas podem vir do banco de dados no futuro)
# Preço médio estimado da unidade no Brasil (Ex: 2023/2024)
PRECOS_MEDIOS = {
    "energia_eletrica": 0.85, # R$/kWh (Tarifa média comercial)
    "gasolina": 5.60,         # R$/Litro
    "diesel": 5.90,           # R$/Litro
    "gas_natural": 4.50       # R$/m³
}

# Fatores de Emissão (kg CO2e por unidade física) baseados no GHG Protocol / SIN
FATORES_EMISSAO = {
    "energia_eletrica": 0.0385, # kgCO2e/kWh (Média do SIN)
    "gasolina": 2.26,           # kgCO2e/Litro
    "diesel": 2.68,             # kgCO2e/Litro
    "gas_natural": 2.02         # kgCO2e/m³
}


def extrair_valor_sped(campos: list) -> float:
    """Busca o valor financeiro em uma linha do arquivo SPED/ECF."""
    for campo in campos:
        # Padrão numérico do SPED (ex: 15430,50 ou 15430.50)
        if re.match(r'^\d+([.,]\d{2})?$', campo) and len(campo) > 2:
            try:
...
def processar_balanco_ecf_para_emissoes(conteudo_arquivo: str) -> Dict:
    """
    Analisa o ECF, identifica gastos operacionais e calcula as emissões estimadas.
    """
    despesas = {
    ...
    for linha in linhas:
        ...
        # Filtro heurístico: Procuramos palavras-chave nas descrições de contas (ex: Registros L300, K355, J050)
        # Ignoramos contas patrimoniais e focamos em despesas
        if "DESPESA" in texto_linha or "CUSTO" in texto_linha or "CONSUMO" in texto_linha:
            valor = extrair_valor_sped(campos)
            
            if valor > 0:
                if "ENERGIA" in texto_linha and "ELETRICA" in texto_linha:
                    despesas["energia_eletrica"] += valor
                elif "GASOLINA" in texto_linha:
                    despesas["gasolina"] += valor
                elif "DIESEL" in texto_linha:
                    despesas["diesel"] += valor
                elif "GAS NATURAL" in texto_linha or "GNV" in texto_linha:
                    despesas["gas_natural"] += valor

    return calcular_relatorio_emissoes(despesas)

def calcular_relatorio_emissoes(despesas_rs: Dict) -> Dict:
    """Converte os gastos em Reais (R$) para Toneladas de CO2e."""
    relatorio = {
        "escopo_1_tco2e": 0.0, 
        "escopo_2_tco2e": 0.0,
        "detalhes": []
    ...
    for categoria, gasto_rs in despesas_rs.items():
        if gasto_rs > 0:
            preco_un = PRECOS_MEDIOS.get(categoria, 1)
            fator_emissao = FATORES_EMISSAO.get(categoria, 0)
            
            # 1. Estima o volume físico: R$ / Preço = Volume (Litros/kWh)
            volume_estimado = gasto_rs / preco_un
            
            # 2. Calcula Emissão: Volume * Fator = kg CO2e -> /1000 = t CO2e
            emissao_tco2e = (volume_estimado * fator_emissao) / 1000

            # 3. Classificação de Escopo
            escopo = "Escopo 2" if categoria == "energia_eletrica" else "Escopo 1"
            
            if escopo == "Escopo 1":
                relatorio["escopo_1_tco2e"] += emissao_tco2e
            else:
                relatorio["escopo_2_tco2e"] += emissao_tco2e
            
            relatorio["detalhes"].append({
                "fonte": categoria.replace("_", " ").title(),
                "gasto_financeiro_rs": round(gasto_rs, 2),
                "volume_estimado": round(volume_estimado, 2),
                "unidade_medida": "kWh" if categoria == "energia_eletrica" else "Litros/m³",
                "emissoes_tco2e": round(emissao_tco2e, 4),
                "escopo": escopo
            })

    relatorio["total_emissoes_tco2e"] = relatorio["escopo_1_tco2e"] + relatorio["escopo_2_tco2e"]
    
    # Arredondando os totais para facilitar a leitura no frontend
    relatorio["escopo_1_tco2e"] = round(relatorio["escopo_1_tco2e"], 4)
    relatorio["escopo_2_tco2e"] = round(relatorio["escopo_2_tco2e"], 4)
    relatorio["total_emissoes_tco2e"] = round(relatorio["total_emissoes_tco2e"], 4)
