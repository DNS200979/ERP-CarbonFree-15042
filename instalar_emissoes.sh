#!/usr/bin/env bash
# ============================================================================
# instalar_emissoes.sh
# Grava a versao CORRETA de app/api/routes/emissoes.py (com a rota importar-ecf)
# direto no caminho certo, sem depender do editor, e reinicia a API.
#
# Uso:
#   1) coloque este arquivo na pasta do projeto (~/Desktop/ERP15042)
#   2) cd ~/Desktop/ERP15042
#   3) bash instalar_emissoes.sh
# ============================================================================
set -e
cd "$(dirname "$0")"

if [ ! -d "app/api/routes" ] && [ -d "$HOME/Desktop/ERP15042/app/api/routes" ]; then
  cd "$HOME/Desktop/ERP15042"
fi
if [ ! -d "app/api/routes" ]; then
  echo "ERRO: nao encontrei app/api/routes/. Coloque este script dentro de ~/Desktop/ERP15042 e rode de novo."
  exit 1
fi

TARGET="app/api/routes/emissoes.py"
echo ">> Alvo: $(pwd)/$TARGET"
cp "$TARGET" "$TARGET.bak.$(date +%s)" 2>/dev/null && echo ">> backup do arquivo atual criado ($TARGET.bak.*)" || true
...
echo ">> gravado: $TARGET"
if grep -q "importar-ecf" "$TARGET"; then
  echo ">> OK: rota importar-ecf presente no arquivo gravado"
else
  echo "ERRO: a gravacao falhou (rota ausente). Nada foi reiniciado."
  exit 1
fi
...
$PY -c "import app.api.routes.emissoes" && echo ">> OK: emissoes.py importa sem erro"
...
echo ">> No navegador: abra http://localhost:8000/docs e de Ctrl+Shift+R (recarga forcada)"
